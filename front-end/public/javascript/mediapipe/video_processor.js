...
// metrics.
  ...
  class VideoProcessor {
     ...
     */
    constructor(downsampleFactor, isFrontFacing, useGlCanvas, useGpuInput) {
      ...
      if (this.useGpuInput_) {
        ...
        // enable mirror mode.
        console.log('Using GL passthrough pipeline; video mirroring must be '
      ...
      window.onload = evt => this.initialize(evt);
      return this;
    }
    ...
    ensureWebGL2(containerElement) {
      ...
      if (!gl) {
        ...
        containerElement.parentNode.innerHTML =
          "WebGL2 did not initialize properly. If you are using Safari, please "
          + "enable WebGL2 following the instructions below and then refresh the "
          + "page<br/><br/>For iOS, go to Settings --> Safari --> Advanced --> "
          + "Experimental Features, and enable WebGL 2.0<br/><br/>For OSX, go to "
          + "Safari --> Preferences --> Advanced, and enable Show Develop menu "
          + "in menubar<br/><br/>Then, go to Develop --> Experimental Features, "
          + "and enable WebGL 2.0";
      }
    }
    ...
    setupCanvases(width, height) {
      ...
      if (!this.downsampleCanvas_ ||
        ...
        this.downsampleCanvas_ = new OffscreenCanvas(
          this.downsampleWidth_, this.downsampleHeight_);
      }
    ...
    initialize() {
      ...
      this.video_.addEventListener(
        'playing', evt => this.getVideoSize(evt), false);
      ...
      window.onresize = evt => this.autoZoomIn(this.canvas_.parentNode);
      ...
      if (this.video_.currentSrc) {
        console.log('Video source tag provided. Not using camera.');
        ...
        }).then(evt => this.onAcquiredUserMedia(evt))
          .catch(e => {
    ...
    onAcquiredUserMedia(stream) {
      ...
      this.video_.onloadedmetadata = () => {this.video_.play(); this.onPlay(0);};
    }
    ...
    onPlay(event) {
      if (this.useGpuInput_) {
        requestAnimationFrame(evt => this.glPassthroughTick(evt));
      } else {
        requestAnimationFrame(evt => this.tick(evt));
      }
    }
    ...
    getVideoSize(event) {
      ...
      this.setupCanvases(640, 480); // videoTarget.videoWidth, videoTarget.videoHeight);
    }
    ...
    tick(timestamp) {
      if (this.video_.currentTime !== this.lastCurrentTime_ &&
        ...
        if (!this.useGlCanvas_) {
          const canvasContext = this.canvas_.getContext('2d');
          if (this.isFrontFacing_) {
            canvasContext.drawImage(this.video_,
        ...
        if (copyContext) {
          if (this.isFrontFacing_) {
            ...
            // respect to that.
            copyContext.drawImage(this.video_,
              -this.downsampleWidth_, 0,
              this.downsampleWidth_, this.downsampleHeight_);
          } else {
        ...
        if (processFrame) {
          ...
          if (this.outputProcessor_) {
            if (result) {
              result["current_mspf"] = this.current_mspf_;
            }
    ...
    glPassthroughTick(timestamp) {
      if (this.video_.currentTime !== this.lastCurrentTime_ &&
        ...
        if (this.processor_) {
          const startTime = performance.now();
          const result = this.processor_.processVideoGl(
            this.video_, startTime);
          this.recordProcessCallMspf(performance.now() - startTime);
          if (this.outputProcessor_) {
            if (result) {
              result["current_mspf"] = this.current_mspf_;
